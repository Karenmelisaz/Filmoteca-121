# -*- coding: utf-8 -*-
"""administrador

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1xO6RvuBXgjxqEbdy7qZjlo5hEBcUwUAd
"""

from reservas_facturacion import leer_reservas
from usuarios import leer_usuarios, buscar_usuario
from funciones import leer_funciones
from cartelera import ordenar_funciones
from asientos import cargar_asientos

ADMIN_USER = "admin"
ADMIN_PASS = "admin123"

def admin():
    print("\n=== ADMIN ===")
    u = input("Usuario: ")
    p = input("Contraseña: ")

    if u != ADMIN_USER or p != ADMIN_PASS:
        print("Acceso denegado.")
        return

    reservas = leer_reservas()
    usuarios = leer_usuarios()

    print("\n--- USUARIOS ---")
    for uu in usuarios:
        print(f"{uu['documento']} - {uu['nombre']} {uu['apellido']} ({uu['vinculo']})")

    total_pago = sum(int(r["precio"]) for r in reservas)

    print("\n--- REPORTES ---")
    print("Total reservas:", len(reservas))
    print("Total pagado:", total_pago)
    print("Promedio diario:", round(total_pago / 3, 2) if reservas else 0)

    conteo = {}
    for r in reservas:
        conteo[r["documento"]] = conteo.get(r["documento"], 0) + 1

    if conteo:
        mayor = max(conteo, key=conteo.get)
        menor = min(conteo, key=conteo.get)
        u1 = buscar_usuario(mayor)
        u2 = buscar_usuario(menor)
        print("Más reservas:", u1["nombre"], u1["apellido"], conteo[mayor])
        print("Menos reservas:", u2["nombre"], u2["apellido"], conteo[menor])

    print("\n--- DISPONIBILIDAD ---")
    for f in ordenar_funciones(leer_funciones()):
        mapa = cargar_asientos(f["function_id"])
        libres = sum(1 for s in mapa if s["estado"] == "O")
        print(f"{f['function_id']} | {f['titulo']} | {libres} disponibles")